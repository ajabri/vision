...
class Downsample3D(nn.Module):
    def __init__(self, pad_type='reflect', filt_size=3, stride=2, channels=None, pad_off=0):
        ...
        self.filt_size = filt_size
        self.pad_off = pad_off
        self.pad_sizes = [int(1.*(filt_size-1)/2), int(np.ceil(1.*(filt_size-1)/2)), int(1.*(filt_size-1)/2), int(np.ceil(1.*(filt_size-1)/2)), 1, 1]
        self.pad_sizes = [pad_size+pad_off for pad_size in self.pad_sizes]
        self.stride = stride
        self.off = int((self.stride-1)/2.)
        self.channels = channels
        ...
        self.pad = Padder(self.pad_sizes, pad_type)
...
class Padder(nn.Module):
    def __init__(self, pad_type, pad_sizes):
        self.pad_type = pad_type
        self.pad_sizes = pad_sizes

    def forward(self, x):
        return F.pad(x, self.pad_sizes, self.pad_type)
