...
def test(val_loader, model, epoch, use_cuda):
    ...
    for batch_idx, (imgs_total, imgs_orig, lbl_set, lbls_tensor, lbls_onehot, lbls_resize, meta) in enumerate(val_loader):
        ...
        for b in range(0, keys.shape[1], bsize):
            ...
            weights = torch.nn.functional.normalize(weights, dim=q_dim, p=q_dim)
        ...
        Is = torch.cat(Is, dim=1)

        t04 = time.time()
        print(t04-t03, 'computed affinities', torch.cuda.max_memory_allocated())
        ...
        for it in range(nstep):
            ...
            T, L, H, W = lbls_base.shape

            lbls_base = torch.nn.functional.pad(lbls_base.transpose(-1,-3), [int(args.radius)]*4).transpose(-2,-1)

            A, weights, idxs = As[it], Ws[it].cuda(), Is[it].cuda()

            lbls = torch.nn.functional.unfold(lbls_base.view(-1, *lbls_base.shape[-3:]), kernel_size=int(args.radius*2 + 1))
            # T x L x P x P x H*W -> L x T x P x P x H*W 
            lbls = lbls.view(T, L, int(2*args.radius+1), int(2*args.radius+1), -1).transpose(0, 1).contiguous()
            lbls = lbls.view(L, -1, A.shape[-2], A.shape[-1])

            predlbls = (torch.gather(lbls, 1, idxs[None].expand(L, -1,-1,-1)) * weights).sum(1)
            ...
            # Save Predictions
            ##################################################################
            predlbls_cp = predlbls.cpu().numpy().copy().transpose(1,2,0)
